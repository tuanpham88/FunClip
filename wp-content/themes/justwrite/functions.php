...
if ( ! function_exists( 'ac_css_files' ) ) {  
	...
	function ac_css_files() {
		...
		// Enqueue
		wp_enqueue_style( 'ac_style', get_stylesheet_uri(), array(), '2.0', 'all' );
		wp_enqueue_style( 'ac_icons', get_template_directory_uri() . '/assets/icons/css/font-awesome.min.css', array(), '4.4.0', 'all' );
...
if ( ! function_exists( 'ac_return_inactive_widgets' ) ) {
	...
	function ac_return_inactive_widgets( $area = '' ) {	
		...
		$notice2 	= '<p class="add-some-widgets container builder">' . esc_html__( 'Disable these demo sections by adding your own in "Main Page - Before posts". You can also disable them from `Customizer > Layout Options > Miscellaneous > Disable demo widgets`.', 'justwrite' ) . '</p>';
